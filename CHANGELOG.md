...
- salle edit : the three refusals join the two fiche forms — an unknown `idS` answers 404 where the return of `loadValues()` was dropped and an empty form appeared under "Modifier une salle", a refused edit answers 403 where a bare message sat above an empty page with a 200 status, and a write failure raises rather than printing a `<div>` before the doctype

...
- lieu, organisateur : an admin note, plain text up to 2000 characters — written in the new "Admin" fieldset of both edit forms, which also holds the status in a nested "Statut" fieldset ; read in a "Note" column of both listings, desktop only and in small italics, the text past 200 characters folded into a "Suite" `<details>`, and under the affiliates or members of the fiche. Admins only, and kept as it is when a lower level saves the fiche ; `CurrentUserEditing::canChangeStatus` becomes `canEditAdminFields`. Stored in the new `admin_note` columns — see [UPGRADE.md](UPGRADE.md) and [docs/lieux-organisateurs.md](docs/lieux-organisateurs.md#note-dadministration)

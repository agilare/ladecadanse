...
### Added
- agenda : on the day's own listing, each event card says where it stands against the moment the page was loaded #51 — a countdown before it starts (`-2h30`, `-45min`), the share elapsed once it is running (`40 %`, rounded to 10 % and kept within 10–90 % so a running event never reads as over), `terminé` once its end time has passed, which also fades the card until it is hovered or one of its links takes focus. An event is placed only where its schedules allow it (see #65) : the countdown needs a start time, the share needs both, `terminé` needs an end time — one that has started with no declared end shows nothing rather than an invented ending, and an out-of-range schedule date is read as the event's own day rather than taken at face value. Off by default behind `EVENT_TIME_STATUS_ENABLED` in `app/env.php`, whose `'preview'` state reserves it to administrators, with a line above the listing saying so and recalling the hour everything is measured against. Another day than today carries none of it — every card there would read as over and fade in one block, or count in days — and the agenda alone carries it for now : search, lieu and organisateur pages and the admin tables are unchanged
- config : feature flags gain a third state through `Ladecadanse\FeatureFlag` — `false`, `'preview'` (administrators only), `true` — so a substantial feature can be tried on the live site before being opened to everyone, which is how they get deployed here anyway. A preview says so in the interface : one that stays silent gets forgotten, and the feature is then believed shipped. `PDF_CONVERSION_ENABLED` is the first to use it
...
### Fixed
- event edit : a new event's flyer is named after the identifier the database actually assigned, not after `MAX(idEvenement) + 1` read before the INSERT — the two drift apart as soon as an event has been deleted, since AUTO_INCREMENT never goes back down, and an event saved as id 4805 got a file named `4804_…`. The mismatch was silent, the column pointing at a file that does exist, but it broke the `{id}_{date}.ext` convention the yearly archiving script relies on, and two simultaneous additions read the same maximum, the second overwriting the first one's image. The columns are now filled by an UPDATE right after the insert ; editing an existing event, which already knew its id, is unchanged
- event copy : the copies' flyers are named after each new event's own identifier, same defect as above — and their source is now the original event's file, read once before the loop, where each copy took the name left by the previous one and copied from that, chaining copies of copies over a date range
...
- event edit : a PDF pasted into "ou coller une URL" is converted like one uploaded through the file field — the two ways of providing an image sat side by side yet only one took PDFs, which read as a bug rather than a limit. The browser cannot handle this case (it would have to read a URL from another domain, which CORS and the site's `connect-src 'self'` forbid), so this path renders server-side with Imagick, whose Ghostscript-backed PDF decoder is acceptable here because the URL field is offered to logged-in accounts only, unlike the file field. Where Imagick is missing the field keeps refusing PDFs, with a message pointing at the upload button rather than a bare failure
- server config : the yearly redirect of event images covers `.webp`, which it had never listed alongside jpg, png and gif — once the January archiving moves a webp flyer into `web/uploads/evenements/<year>/`, its former URL answered 404 where the other formats got their 301 ; harmless while webp was rare, but PDF uploads are now converted to webp in the browser, so it is about to become the most common format. The range of years deliberately stops at the previous one: `Evenement::getFilePath()` only prefixes a year once it is over, so an image of the current year still lives at the root and listing it here would turn its 404s into redirects to a directory that does not exist yet
...
- events, lieux, users edit : deselecting an organisateur now sticks after a validation error — the multiple select merged the posted selection with the organisateurs read back from the database, so one just removed came back ticked at every redisplay, and could not be removed at all while another error blocked saving ; the hidden `formulaire=ok` witness now decides alone, a fully deselected multiple select posting no key at all, on which `isset($_POST[…])` would fall back to the database ; saving was already correct, the defect was purely in the redisplay
...
- tests : unit coverage for the lieu and organisateur options and for the 301 of every moved page ; the `site` suite covers the admin events screen — access, the three filters, their memory, the sort links, the CSRF token and the folded blocks — and the organisateurs preselection of the three edit forms, two tests each : one organisateur removed, then all of them, the second being the one the witness exists for

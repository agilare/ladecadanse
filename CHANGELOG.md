...
### Changed
- pages moved, each with a 301 : register, password reset and logout join `user/` next to login and dashboard (#121, #123), `organisateur-edit.php` becomes `organisateur/edit.php` #115, `lieu-edit.php` becomes `lieu/edit.php` #117, `admin/gererEvenements.php` becomes `admin/events.php` #125 — each request is now processed before the first byte of HTML, `OrganisateurEdition` and `LieuEdition` move to PDO, the admin listing to `EvenementCollection` — see [UPGRADE.md](UPGRADE.md)
- lieu edit : the form is rewritten on the organisateur one #117 — processing before the first byte of HTML, four fieldsets in a reading order (identity, practical info, relations, photo), categories as a Select2 multiselect, the gallery gone with the feature it served, and the left calendar column dropped ; what the two forms share — reading back the stored state, the guard on reserved fields, the image lifecycle, the delegation of errors — becomes `FicheEdition`, and the image field a single partial included by both — see [docs/lieux-organisateurs.md](docs/lieux-organisateurs.md)
- lieu : `lieu.determinant` becomes `preposition_nom` and `lieu.categorie` becomes `categories`, `adresse` goes to 255 characters, the optional columns accept `NULL`, and `photo2` and `actif` are dropped — the first proposed by no form, the second an inert duplicate of `statut` ; statuts, categories and field lengths move from `app/config.php` globals to `Lieu::STATUTS`, `Lieu::CATEGORIES` and `Lieu::FIELDS` — see [UPGRADE.md](UPGRADE.md)
- lieu edit : latitude and longitude become `Coordinates`, which carries their three rules in one place — the decimal comma of European keypads, the globe's bounds, and "both or neither" — where they lived in the form class, its validation and a closure of the template
...
- organisateur edit : who may edit a fiche is asked once, in `Authorization::isPersonneAllowedToEditOrganisateur()`, by the form and by the fiche's own link ; field lengths live in `Organisateur::FIELDS`, where the form takes its `maxlength` and the server its validation bounds ; refusing answers 403, a request naming no organisateur 400 and one naming an unknown fiche 404, all rendered inside the site's page — see [docs/lieux-organisateurs.md](docs/lieux-organisateurs.md)
- edition : the HTML sanitizer applied to texts written outside the site becomes `UserHtmlSanitizer`, where `OrganisateurEdition` and `lieu-text-edit.php` each carried the same configuration — two places to loosen the rule, one to remember to tighten it
- forms : the preview of an image already uploaded is `.existing_img` on the four edit forms and in the stylesheets — `.supImg` named the delete checkbox rather than what the block shows
...
- lieu edit : a lieu created in Oulens-sur-Lucens is no longer filed under the Geneva region — a hardcoded exception meant for Nyon named localité 529, and Nyon is 513 ; the region now follows the localité's canton alone, `regions_covered` already saying what that exception tried to say. Modifying a fiche no longer makes the editor its author, `?action=editer` without an `idL` answers 400 and an unknown `idL` 404 instead of an UPDATE touching no row and reporting success ; the `maxlength` of nom, adresse, préposition and site web say what the server actually accepts, where the four disagreed with it
- lieu edit, organisateur edit : an unknown `?action=` answers 400 instead of a 500 — the validator signalled it by an exception nothing caught, on the very urls bots produce in bulk ; `QueryParamValidator::enumFromQuery()` separates the missing parameter, which falls back on the default, from the unknown one
- lieu edit : a lieu without coordinates keeps an empty pair rather than a `0` that reads as a point in the Gulf of Guinea ; the page took its list of accepted actions from a global leaking out of `app/config.php`, where it declared its own and left it unused
...
- admin, admin events : the "par" column reads `anonyme` for an event submitted without an account, where both listings rendered a link to `/user/dashboard.php?idP=0` with neither label nor tooltip — the public form accepts proposals from visitors, so `idPersonne` is legitimately 0
...
- lieu edit : the lieu being written is the one named by the url, whose permission was checked, and no longer the one named by a hidden `idLieu` field — anyone allowed to edit one lieu could edit every other by tampering with it #117. The fields reserved to editors — nom, préposition, catégories, organisateurs — travelled the same way as hidden inputs and are now read back from the database ; the lieu's own INSERT and UPDATE are prepared statements with named columns, instead of a SET built from the keys the form posted — see [UPGRADE.md](UPGRADE.md)

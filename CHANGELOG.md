...
- lieu edit : the `?action=` parameter was validated against an undefined variable, so every value was accepted ; a lieu without coordinates keeps an empty pair rather than a `0` that reads as a point in the Gulf of Guinea

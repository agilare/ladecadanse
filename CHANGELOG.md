...
### Added
- events : two categories join the five an event can be filed under — `concerts`, which "fêtes" had been absorbing since always (the form's own tooltip said so), and `cours`, for the courses, workshops and residencies that sat in "divers" next to conferences and flea markets. Outside the preview an event filed under `concerts` is ranked and shown as a "fête", one under `cours` as a "divers" : it is a filing, not a masking — `evenement.genre` stays a `varchar(20)`, nothing is migrated, and turning the flag back down loses no re-classification. The list leaves `app/config.php` for `Ladecadanse\EventCategory`, which also carries the fallback and the sort rank the three `ORDER BY CASE` had each spelled out ; off by default behind `EVENT_NEW_CATEGORIES_ENABLED` — see [docs/evenements.md](docs/evenements.md)

...
- tests : unit coverage for the lieu and organisateur options and for the 301 of every moved page ; the `site` suite covers the admin events screen and the organisateurs preselection of the three edit forms
- docker : a freshly created database gets the migrations the dump does not carry yet — the 3.8 and 3.9 indexes and the `bot_monitor` table were missing from every volume created since, `evenement` running its searches without a single one of them ; the ones already folded into `ladecadanse.sql` stay out, a replayed `v3-12-0_localite-france.sql` adding a second "Ailleurs en France" — see the [README](README.md#base-de-données)

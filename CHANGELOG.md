...
- lieu edit, organisateur edit : an unknown `?action=` answers 400 instead of a 500 — the validator signalled it by an exception nothing caught, on the very urls bots produce in bulk ; `QueryParamValidator::enumFromQuery()` separates the missing parameter from the unknown one

...
class Element
{
    ...
    function load(): void
    {
        $sql = "SELECT * FROM " . $this->connector->sanitize($this->table) . " WHERE id" . $this->connector->sanitize(ucfirst((string) $this->table)) . "=" . (int) $this->id;
    ...
    {
        $sql = "INSERT INTO " . $this->connector->sanitize($this->table) . " SET ";
        foreach ($this->valeurs as $nom => $val)
        {
            $sql .= $this->connector->sanitize($nom) . "='" . $this->connector->sanitize($val) . "', ";
        }
    ...
    function update(): bool
    {
        ...
        $sql = mb_substr($sql, 0, -2);
        $sql .= " WHERE id" . $this->connector->sanitize(ucfirst((string) $this->table)) . "=" . (int) $this->id;
    ...
    function getMaxId()
    {
        $req_max_id = $this->connector->query("SELECT MAX(id" . $this->connector->sanitize(ucfirst((string) $this->table)) . ") AS max_id FROM " . $this->connector->sanitize($this->table));

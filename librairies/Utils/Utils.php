...
class Utils
{
    public static function urlQueryArrayToString(array $get, $sauf = ""): string
    {
        ...
        if (!is_array($sauf))
        {
            ...
            {
                if ($nom != $sauf && !($nom == 'tri_agenda' && in_array($valeur, ['', 'dateAjout'])) && !($nom == 'genre' && empty($valeur)) && !($nom == 'sem' && empty($valeur)))
                {
                    $afficher .= $nom . "=" . $valeur . "&";
                }
            }
        ...
        else
        {
            foreach ($get as $nom => $valeur)
            {
                if (!in_array($nom, $sauf) && !($nom == 'tri_agenda' && in_array($valeur, ['', 'dateAjout'])) && !($nom == 'genre' && empty($valeur)) && !($nom == 'sem' && empty($valeur)))
                {
                    $afficher .= $nom . "=" . $valeur . "&";
                }
            }

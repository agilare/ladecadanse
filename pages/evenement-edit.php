...
//        }
	...
	if ($verif->nbErreurs() === 0)
	{
		...
		if ($get['action'] == 'insert')
		{
			...
			*/
			if ($connector->query($sql_insert))
			{
				...
				$_SESSION['evenement-edit_flash_msg'] = "L'événement a été créé. <a href='/pages/agenda.php?courant=".$champs['dateEvenement']."#event-".$req_id."'>Voir dans l'agenda</a>";	
                ...
                if (!isset($_SESSION['Sgroupe']))
                {
                    ...
                    $contenu_message .= $url_site."/pages/evenement.php?idE=".$req_id;
                    $contenu_message .= "\n\n";
		...
		elseif ($get['action'] == 'update')
		{
			...
			*/
			if ($req_update)
			{
				...
				$lienLieu = '';
				if (!empty($lieu))
					$lienLieu = " au <a href=\"/pages/lieu.php?idLieu=".$lieu."\"> lieu ".$lieu."</a>";
                ...
                $_SESSION['evenement-edit_flash_msg'] = "L'événement a été modifié.$confirmation_flash_msg<br><a href='/pages/agenda.php?courant=".$champs['dateEvenement']."#event-".$req_id."'>Voir dans l'agenda</a>";    
		...
		*/
		...
		unset($_POST); // ?
        $logger->log('global', 'activity', "[evenement-edit] ".$get['action']." of \"".$champs['titre']."\" in ".$champs['nomLieu']." /pages/evenement.php?idE=".$evenement['idEvenement'], Logger::GRAN_YEAR);                

        if (isset($_SESSION['Sgroupe']))
        {
            header("Location: /pages/evenement.php?idE=".$req_id); die();
        }
        else
        {
...
if (!$action_terminee)
{
    ...
    */
    ...
    /*			$tab = explode("-", $affEven['dateEvenement']);
        ...
        if ($_SESSION['Sgroupe'] <= 10)
        {
            ...
            if ($_SESSION['Sgroupe'] <= 1)
            {
                ...
                <a href=\"/pages/multi-suppr.php?action=confirmation&amp;type=evenement&amp;id=".$get['idE']."&token=".SecurityToken::getToken()."\" title=\"Supprimer l'événement\" onclick=\"return confirm('Voulez-vous vraiment supprimer cet événement ?');\">
                Supprimer</a>
                </li>";
            }

            $aff_actions .= "<li class=\"action_copier\">
                <a href=\"/pages/evenement-copy.php?idE=".$get['idE']."\" title=\"Copier l'événement vers une autre date\">Copier vers d'autres dates</a></li></ul>";
        }
    } // action editer
	...
	{
        $aff_titre .= '<h2>Modifier <a style="font-size:0.7em" href="/pages/evenement.php?idE='.$get['idE'].'" title="Fiche de l\'événement" >'.$champs['titre'].'</a></h2>';
...
<form method="post" id="ajouter_editer" class="submit-freeze-wait" enctype="multipart/form-data" action="<?php echo basename(__FILE__)."?action=".$act ?>">
    ...
    <ul style="line-height:1.2em">
        <li style="margin:6px 2px;">n’est pas déjà présent dans l’<a href="/pages/agenda.php" target="_blank">agenda</a></li>
        <li style="margin:6px 2px;">respecte notre <b><a href="/pages/articles/charte-editoriale.php" target="_blank">charte&nbsp;éditoriale</a></b></li>
    </ul>
    ...
    <fieldset id="references">
        ...
        <div class="guideChamp">L’événement figurera dans la page de ces <a href="/pages/organisateurs.php" target="_blank">organisateurs</a>. Si vous souhaitez que votre organisation soit listée, <a href="/pages/contacteznous.php" target='_blank'>demandez-nous</a></div>
        </p>
    </fieldset>

...
if (($get['action'] == 'editer' || $get['action'] == 'update'))
{
	...
	if ($_SESSION['Sgroupe'] <= 6 || $authorization->isPersonneAffiliatedWithLieu($_SESSION['SidPersonne'], $get['idL']) || $authorization->isPersonneInLieuByOrganisateur($_SESSION['SidPersonne'], $get['idL']))
	{
		...
		if ($_SESSION['Sgroupe'] < 2)
		{
			...
			<a href=\"/pages/multi-suppr.php?type=lieu&amp;id=".$get['idL']."&token=".SecurityToken::getToken()."\">Supprimer</a></li>";
			$titre_actions .= '</ul>';
		}
...
<p>Si vous souhaitez modifier le nom du lieu, ses catégories, ses organisateurs ou les photos de la galerie, merci de nous <a href="/pages/contacteznous.php">contacter</a></p>
<fieldset>

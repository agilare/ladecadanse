...
if ($get['element'] == "description")
{
	...
	foreach ($tab_nblignes as $nbl)
	{
		...
		echo '><a href="/pages/admin/gerer.php?'.Utils::urlQueryArrayToString($get, "nblignes").'&nblignes='.$nbl.'">'.$nbl.'</a></li>';
	}
	echo '</ul>';
echo '<div class="spacer"></div>';
	...
	while($tab_desc = $connector->fetchArray($req_des))
	{
		...
		{
			echo "<td><a href=\"/pages/multi-description.php?action=editer&idL=".$tab_desc['idLieu']."&idP=".$tab_desc['idPersonne']."\" title=\"Éditer le lieu\">".$iconeEditer."</a></td>";
		}
		echo "</tr>";
...
echo '<div class="spacer"></div>';
	...
	while($tab_lieux = $connector->fetchArray($req_lieux))
	{
		...
		<td><a href=\"/pages/lieu.php?idL=".$tab_lieux['idLieu']."\" title=\"Voir la fiche du lieu :".sanitizeForHtml($tab_lieux['nom'])."\">".sanitizeForHtml($tab_lieux['nom'])."</a></td>
		<td class=\"tdleft\"><ul>";
		...
		if ($_SESSION['Sgroupe'] <= 4)
		{
			echo "<td><a href=\"/pages/lieu-edit.php?action=editer&idL=".$tab_lieux['idLieu']."\" title=\"Éditer le lieu\">".$iconeEditer."</a></td>";
		}
		echo "</tr>";
...
echo '<div class="spacer"></div>';
	...
	while($tab = $connector->fetchArray($req_lieux))
	{
		...
		if ($_SESSION['Sgroupe'] <= 4)
		{
			echo "<td><a href=\"/pages/organisateur-edit.php?action=editer&idO=".$tab['idOrganisateur']."\" title=\"Éditer\">".$iconeEditer."</a></td>";
		}
		echo "</tr>";
...
echo '<div class="spacer"></div>';
	...
	while($tab_comm = $connector->fetchArray($req_comm))
	{
		...
		echo "<td><a href=\"/pages/user.php?idP=".$tab_comm['idPersonne']."\" title=\"Voir l'auteur\">".$tab_pers['pseudo']."</a></td>";
		...
		if ($_SESSION['Sgroupe'] <= 4)
		{
			echo "<td><a href=\"/pages/multi-comment.php?action=editer&idC=".$tab_comm['idCommentaire']."\">".$iconeEditer."</a></td>";
		}
...
echo '<div class="spacer"></div>';
	...
	while ($tab_pers = $connector->fetchArray($req_pers))
	{
		...
		<td style='width:20%'><a href=\"/pages/user.php?idP=".$tab_pers['idPersonne']."\" title=\"Voir le profile :".sanitizeForHtml($tab_pers['pseudo'])."\">".sanitizeForHtml($tab_pers['pseudo'])."</a></td>
		...
		{
			echo "<td><a href=\"/pages/user-edit.php?action=editer&idP=".$tab_pers['idPersonne']."\" title=\"Éditer\">".$iconeEditer."</a></td>";
		}
		echo "</tr>";

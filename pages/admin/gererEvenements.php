	...
	if ($verif->nbErreurs() === 0)
	{
		...
		foreach ($evenements as $idEven_courant)
		{
			...
			*/
			if ($req_update)
			{
				HtmlShrink::msgOk('Mise à jour de <a href="/pages/evenement.php?idE='.$idEven_courant.'">'.$tab_even['titre'].'</a> le <a href="/pages/agenda.php?courant='.$tab_even['dateEvenement'].'">'.date_fr($tab_even['dateEvenement'], "annee").'</a> réussie');
...
foreach ($tab_nblignes as $nbl)
{
	...
	echo '><a href="/pages/admin/gererEvenements.php?'.Utils::urlQueryArrayToString($get, "nblignes").'&nblignes='.$nbl.'">'.$nbl.'</a></li>';
}
echo '</ul>';
?>
...
while ($tab_even = $connector->fetchArray($req_evenement))
{
	...
	echo "	<td><a href=\"/pages/evenement.php?idE=".$tab_even['idEvenement']."\" title=\"Voir la fiche de l'événement\" class='titre'>".sanitizeForHtml($tab_even['titre'])."</a></td>	<td>".$nomLieu."</td>
	...
	{
		echo '<td style="text-align:center"><a href="/pages/evenement-edit.php?action=editer&idE='.$tab_even['idEvenement'].'" title="Éditer l\'événement">'.$iconeEditer.'</a></td>';
	}

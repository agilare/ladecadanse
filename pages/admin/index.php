...
while($tab_pers = $connector->fetchArray($req_get))
{
	...
	<td><a href=\"/pages/user.php?idP=".$tab_pers['idPersonne']."\" >".sanitizeForHtml($tab_pers['pseudo'])."</a></td>
	...
	{
		echo "<td><a href=\"/pages/user-edit.php?action=editer&amp;idP=".$tab_pers['idPersonne']."\" title=\"Modifier\">".$iconeEditer."</a></td>";
	}
	echo "</tr>";
...
while($tab_comm = $connector->fetchArray($req_comm))
{
	...
	{
		$nom_auteur = "<a href=\"/pages/user.php?idP=".$tab_comm['idPersonne']."\"
	...
	{
		echo "<td><a href=\"/pages/multi-comment.php?action=editer&amp;idC=".$tab_comm['idCommentaire']."\" title=\"Éditer la brêve\">".$iconeEditer."</a></td>";
	}
...
<?php if ($_SESSION['Sgroupe'] < 4) { ?>  
<p><a href="/pages/admin/gerer.php?element=personne">Gérer les personnes</a></p>
<?php } ?>
...
while($tab_even = $connector->fetchArray($req_getEvenement))
{
	...
	echo "<td><a href=\"/pages/evenement.php?idE=".$tab_even['idEvenement']."\" title=\"Voir la fiche de l'événement\" class='titre'>".sanitizeForHtml($tab_even['titre'])."</a></td>
	<td>".$nomLieu."</td>
	...
	{
		$nom_auteur = "<a href=\"/pages/user.php?idP=".$tab_even['idPersonne']."\"
		title=\"Voir le profile de la personne\">".sanitizeForHtml($tab_auteur['pseudo'])."</a>";
	}
	echo "<td>".$nom_auteur."</td>";
...
Rien
<?php } ?>
<p><a href="/pages/admin/gererEvenements.php">Gérer les événements</a></p>
...
while($tab = $connector->fetchArray($req_fav_even))
{
	...
	echo "<td><a href=\"/pages/evenement.php?idE=".$tab['idEvenement']."\"
	...
	{
		$nom_auteur = "<a href=\"/pages/user.php?idP=".$tab['idPersonne']."\"
		title=\"Voir le profile de la personne\">".sanitizeForHtml($tab_auteur['pseudo'])."</a>";
	}
	echo "<td>".$nom_auteur."</td>";
...
while ($tab_desc = $connector->fetchArray($req_getDes))
{
	...
	{
		$nom_auteur = "<a href=\"/pages/user.php?idP=".$tab_desc['idPersonne']."\"
		title=\"Voir le profile de la personne\">".sanitizeForHtml($tab_auteur['pseudo'])."</a>";
	}

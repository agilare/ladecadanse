...
FROM evenement WHERE idEvenement=".$get['idE'])));
		...
		$_SESSION['copierEvenement_flash_msg']['msg'] = '<p style="margin:4px 0">L\'événement <a href="/pages/evenement.php?idE='.$get['idE'].'"><strong>'.sanitizeForHtml($tab_champs['titre']).'</strong> du '.date_fr($tab_lieu['dateEvenement']).'</a> a été copié vers les dates suivantes :</p>';
...
/* 			//FAIRE BOUCLE DE DATE1 A DATE2
			...
			*/
			if ($connector->query($sql_insert))
			{
				...
				$edition = " <a href=\"/pages/evenement-edit.php?action=editer&idE=".$nouv_id."\" title=\"Éditer l'événement\">".$iconeEditer."</a>";
				
                $hor_compl = '';
                if (!empty($tab_champs['horaire_complement']))
                    $hor_compl = "<br>".$tab_champs['horaire_complement'];
                
				$_SESSION['copierEvenement_flash_msg']['table'] .= '<tr><td><a href="/pages/evenement.php?idE='.$nouv_id.'">'.sanitizeForHtml($tab_champs['titre'])."<br>".date_fr(date('Y-m-d', $dateIncrUnix)).'</a></td><td>'.$hor_debfin.$hor_compl.'</td><td><a class="action_editer" href="/pages/evenement-edit.php?action=editer&idE='.$nouv_id.'" title="Modifier cet événement">Modifier</a>&nbsp;&nbsp;<a href="/pages/evenement-edit.php?action=editer&idE='.$nouv_id.'" title="Modifier cet événement dans un nouvel onglet" target="_blank"><i class="fa fa-external-link" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="#" id="btn_event_del_'.$nouv_id.'" class="btn_event_del action_supprimer" data-id='.$nouv_id.'>Supprimer</a></td></tr>';
...
if (isset($get['idE']))
{
		...
		if ($affEven = $connector->fetchArray($req_getEven))
		{
			...
			include($rep_absolu."pages/_evenement.inc.php");
		}
		else
		{

...
$url_retour = $url_site.'/pages/evenement.php?idE='.$get['id'].'#commentaires';
if ($get['element'] == 'lieu')
{
	$url_retour = '/pages/lieu.php?idL='.$get['id'].'&complement=commentaires#ajouter_editer';
}
		...
		*/
		if ($get['action'] == 'insert')
		{
			...
			//
			{
				...
				{
					HtmlShrink::msgOk("Commentaire de <a href=\"/pages/evenement.php?idE=".$get['id']."\" title=\"Voir la fiche de l'événement\">".$iconeVoirFiche."l'événement</a> ajouté");
				}
				else
				{
					HtmlShrink::msgOk("Commentaire du  <a href=\"/pages/lieu.php?idL=".$get['id']."&amp;complement=commentaires#menu_complement\" title=\"Voir la fiche du lieu\">".$iconeVoirFiche."lieu</a> ajouté");
				}
...
if ($get['action'] == 'editer' && isset($get['idC']))
{
	...
	if ($_SESSION['SidPersonne'] == $get['idP'] || $_SESSION['Sgroupe'] <= 4)
	{
		...
		echo " <a href=\"/pages/multi-suppr.php?action=confirmation&amp;type=commentaire&amp;id=".$get['idC']."&token=".SecurityToken::getToken()."\" title=\"Supprimer le commentaire\"  onclick=\"return confirm('Voulez-vous vraiment supprimer ce commentaire ?');\">Supprimer</a>";
		echo "</li>";
	}
} // if GET action

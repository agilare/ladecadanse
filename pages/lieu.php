...
include($rep_absolu."pages/_menulieux.inc.php");
...
if ((isset($_SESSION['Sgroupe']) && $_SESSION['Sgroupe'] <= 12))
{
	...
	if ($nb_favori == 0)
	{
		$menu_actions .= '<li><a href="/pages/multi-star.php?action=ajouter&amp;element=lieu&amp;idL='.$get['idL'].'"
	title="Ajouter à vos favoris">'.$icone['ajouter_favori'].'Ajouter aux favoris</a></li>';
	}
	else
	{
		$menu_actions .= '<li><a href="/pages/multi-star.php?action=supprimer&amp;element=lieu&amp;idL='.$get['idL'].'"
...
{
	$action_ajouter = '<li class="action_ajouter"><a href="/pages/evenement-edit.php?idL='.$get['idL'].'" title="ajouter un événement à ce lieu">Ajouter un événement à ce lieu</a></li>';
}
...
{
	$action_editer = '<li class="action_editer"><a href="/pages/lieu-edit.php?action=editer&amp;idL='.$get['idL'].'">Modifier ce lieu</a></li>';
}
...
<?php	echo $lieu_status ?>
	...
	<div id="fiche"<?php echo $class_vide; ?>>
		...
		<div id="medias">
            ...
            { 
            ?>            
			<?php echo (!$photo_principale)?'<p style="font-size:0.9em;padding:2em 0.5em;line-height:1.2em">Vous gérez ce lieu ? <a href="/pages/user-register.php">Inscrivez-vous</a> pour pouvoir ajouter ou modifier les informations et des photos</p>':""; ?>
            <?php } ?>
			</div>
			<div class="spacer"><!-- --></div>
...
//		echo $sql_salle;
		...
		if ($connector->getNumRows($req_salle) > 0)
		{
			...
			while ($tab_salle = $connector->fetchArray($req_salle))
			{
				...
				{
					$salles .= '<a href="/pages/lieu-salle-edit.php?action=editer&amp;idS='.$tab_salle['idSalle'].'">'.$iconeEditer.'</a>';
				}
		...
		if (isset($_SESSION['Sgroupe']) && $_SESSION['Sgroupe'] <= 6)
		{
			$salles .= '<a href="/pages/lieu-salle-edit.php?idL='.$get['idL'].'">'.$icone['ajouts'].'ajouter une salle</a>';
		}
		...
		if ($connector->getNumRows($req) > 0)
		{
			...
			{
				$organisateurs .= '<li><a href="/pages/organisateur.php?idO='.$tab['idOrganisateur'].'" >';
...
<div id="descriptions">
        ...
        if ($descriptions->loadByType($get['idL'], $type))
        {
            foreach ($descriptions->getElements() as $id => $des)
            {
                        ...
                        )
                {
                    ...
                    $editer .= '<a href="/pages/multi-description.php?action=editer&amp;type='.$type.'&amp;idL='.$get['idL'].'&amp;idP='.$des->getValue('idPersonne').'">'.$iconeEditer.'Modifier</a>';
                    $editer .= '</span>';
	...
	{
		echo "<a href=\"/pages/multi-description.php?idL=".$get['idL']."&amp;type=description\">".$icone['ajouter_texte']." Ajouter une description (avis)</a><br>";
	}
	...
	if (isset($_SESSION['Sgroupe']) &&
            ...
            && $nb_pres == 0)
	{
		echo "<a href=\"/pages/multi-description.php?idL=".$get['idL']."&amp;type=presentation\">".$icone['ajouter_texte']." Ajouter une présentation</a>";
	}
	?>
...
if ($get['complement'] == 'evenements')
{
	$lien_rss_evenements = '<a href="/pages/rss.php?type=lieu_evenements&amp;id='.$get['idL'].'"
...
{ */
	...
	if ($evenements->getNbElements() > 0)
	{
		...
		foreach ($genres_even as $g)
		{
			...
			if ($g == $get['genre_even'])
			{
				$menu_genre .= " class=\"ici\"><a href=\"/pages/lieu.php?idL=".$get['idL']."&amp;genre_even=".urlencode($g)."#prochains_even\" title=\"".$g."\">";
				if ($g == "fête")
				{
					$g .= "s";
				}
			...
			{
				$menu_genre .= "><a href=\"/pages/lieu.php?idL=".$get['idL']."&amp;genre_even=".$g."#prochains_even\" title=\"".$g."\">";
				if ($g == "fête")
				{
					$g .= "s";
				}
	...
	foreach ($evenements->getElements() as $id => $even)
	{
		...
		{	
			...
			{
				//$continuer = "<span class=\"continuer\"><a href=\"/pages/evenement.php?idE=".$even->getValue('idEvenement')."\" title=\"Voir la fiche complète de l'événement\"> Lire la suite</a></span>";
...
?>		
			...
			<h3 class="summary">
			<?php
			$titre_url = '<a class="url" href="/pages/evenement.php?idE='.$even->getValue('idEvenement').'" title="Voir la fiche de l\'événement">'.Evenement::titre_selon_statut(sanitizeForHtml($even->getValue('titre')), $even->getValue('statut')).'</a>';
			echo $titre_url; ?>
			</h3>
			...
			<ul>
				...
				<li><a href="/pages/evenement-copy.php?idE=<?php echo $even->getValue('idEvenement') ?>" title="Copier cet événement"><?php echo $iconeCopier ?></a></li>
				<li><a href="/pages/evenement-edit.php?action=editer&amp;idE=<?php echo $even->getValue('idEvenement') ?>" title="Éditer cet événement"><?php echo $iconeEditer ?></a></li>
...
{ */
	...
	{
	?>
		<form method="post" id="ajouter_editer" action="pages/multi-comment.php?action=insert&amp;element=lieu&amp;id=<?php echo $get['idL'] ?>">
...
<a href="/pages/user-register.php" title="Formulaire d'inscription"><strong>Créez un compte</strong></a> afin de pouvoir ajouter vos commentaires.
</p>
<?php
	} // if login
	?>

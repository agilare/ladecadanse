		...
		*/
		if ($req_insert)
		{
			...
			if ($connector->query($sql_update))
			{
				...
				$contenu_message .= "\n\n";
				$contenu_message .= "Pour vous connecter : ".$url_site."/pages/user-login.php";
				$contenu_message .= "\n\n";
				$contenu_message .= "Vous pouvez compléter votre profil sur votre page de membre : ";
				$contenu_message .= $url_site."/pages/user.php?idP=".$req_id;
				$contenu_message .= "\n\n";
				...
				HtmlShrink::msgOk("<strong>Votre compte".$compte_organisateur." a été créé</strong>; vous pouvez maintenant vous <a href=\"".$url_site."/pages/user-login.php\">connecter</a> avec l'identifiant et le mot de passe que vous venez de saisir.
				<br />Un e-mail de confirmation récapitulant vos données d'accès vous a été envoyé à l'adresse : ".$tab_pers['email']);

                $logger->log('global', 'activity', "[user-register] by ".$tab_pers['pseudo']." (".$tab_pers['email'].") in group ".$tab_pers['groupe']." ".$url_site."/pages/user.php?idP=".$req_id, Logger::GRAN_YEAR);
			}
...
<h2>user-register à La décadanse</h2>
...
<form method="post" id="ajouter_editer" class="submit-freeze-wait" action="<?php echo $_SERVER['PHP_SELF']."?action=".$act; ?>" onsubmit="return valideruser-edit();">
    ...
    <p>Avant de vous inscrire en tant qu'Organisateur, veillez svp à ce que les événements que vous souhaitez ajouter respectent notre <b><a href="/pages/articles/charte-editoriale.php">charte&nbsp;éditoriale</a></b>.</p>
...
<fieldset>
	...
	<fieldset class="affiliation" id="user-register_references" >
        ...
        <div class="guide_affiliation">Si vous avez choisi <b>Acteur culturel</b>, merci d'indiquer à quel association, collectif, lieu, etc. vous appartenez.<br>Ainsi, une fois votre compte créé, vous pourrez modifier les informations du <a href="/pages/lieux.php" target="_blank">Lieu</a> et/ou <a href="/pages/organisateurs.php" target="_blank">Organisateur</a> sur La décadanse (données pratiques, images, présentations)</div>
		<p>

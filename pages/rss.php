...
else if ($get['type'] == "evenements_auj")
{
	...
	while($tab_even = $connector->fetchArray($req_even))
	{
		...
		$items .= "<link>".$url_site."/pages/evenement.php?idE=".$tab_even['idEvenement']."</link>\n";
		$items .= "<comments>".$url_site."/pages/evenement.php?idE=".$tab_even['idEvenement']."#commentaires</comments>\n";
		$items .= "<description><![CDATA[";

		if ($tab_even['idLieu'] != 0)
		{
			$nom_lieu = "<a href=\"".$url_site."/pages/lieu.php?idL=".$tab_even['idLieu']."\"
...
else if ($get['type'] == "lieu_evenements")
{
	...
	$channel .= '<link>'.$url_site.'/pages/lieu.php?idL='.$get['id'].'</link>';
...
else if ($get['type'] == "evenement_commentaires")
{
	...
	$channel .= '<link>'.$url_site.'/pages/evenement.php?idE='.$get['id'].'</link>';
	...
	while($tab_comm = $connector->fetchArray($req_comm))
	{
		...
		$items .= "<link>".$url_site."/pages/evenement.php?idE=".$tab_comm['idEvenement']."</link>\n";
...
else if ($get['type'] == "lieux_descriptions")
{
	...
	$channel .= '<link>'.$url_site.'/pages/lieux.php</link>';
	...
	while($tab = $connector->fetchArray($req))
	{
		...
		$items .= "<link>".$url_site."/pages/lieu.php?idL=".$tab['idLieu']."</link>\n";
...
else if ($get['type'] == "organisateur_evenements")
{
	...
	$channel .= '<link>'.$url_site.'/pages/organisateur.php?idO='.$get['id'].'</link>';

...
if (!$even->getValues() || in_array($even->getValue('statut'), ['inactif', 'propose']) )
{
	...
	else
	{
		...
		echo file_get_contents($rep_absolu.$rep_absolu."404.php");
		exit;
	}
}
...
if ($connector->getNumRows($req_even) > 0)
{
	...
	*/
	...
	while ($tab_even = $connector->fetchArray($req_even))
	{
		...
		$courant = "/pages/evenement.php?idE=".$tab_even['idEvenement']."&amp;tri=".$get['tri'];
...
?>
 ...
 ?>>
            ...
            <span class="category"><?php echo ucfirst(Evenement::nom_genre($even->getValue('genre'))); ?></span>, <?php echo '<a href="/pages/agenda.php?courant='.$even->getValue('dateEvenement').'">'.date_fr($even->getValue('dateEvenement'), "annee", "", "", false).'</a>';
 ?>
</h2>
	...
	{
		echo '<li><a href="/pages/evenement-email.php?idE='.$get['idE'].'" title="Envoyer l\'événement par email">'.$icone['envoi_email'].'Envoyer à un ami</a></li>';
		...
		if ($nb_favori == 0)
		{
			echo '<li><a href="/pages/multi-star.php?action=ajouter&amp;element=evenement&amp;idE='.$get['idE'].'">'.$icone['ajouter_favori'].'Favoris</a></li>';
		}
		else
		{
			echo '<li><a href="/pages/multi-star.php?action=supprimer&amp;element=evenement&amp;idE='.$get['idE'].'">'.$icone['supprimer_favori'].'Favoris</a></li>';
		}
	}
	...
	|| (isset($_SESSION['Saffiliation_lieu']) && $even->getValue('idLieu') != '' && $even->getValue('idLieu') == $_SESSION['Saffiliation_lieu'])
		...
		?>
			<li><a href="/pages/evenement-copy.php?idE=<?php echo $get['idE'] ?>" title="Copier cet événement"><?php echo $iconeCopier ?>Copier vers d'autres dates</a></li>
		<?php
		}
	...
	|| isset($_SESSION['SidPersonne']) && $authorization->isPersonneInEvenementByOrganisateur($_SESSION['SidPersonne'], $even->getId())	
		...
		{
		?>
			<li><a href="/pages/evenement-edit.php?action=editer&amp;idE=<?php echo $get['idE'] ?>"><?php echo $iconeEditer ?>Modifier</a></li>
		<?php
		}
		?>
			...
			<li><a href="/pages/evenement_ics.php?idE=<?php echo $get['idE'] ?>" title="Exporter au format iCalendar dans votre agenda"><i class="fa fa-calendar-plus-o fa-lg"></i>
iCal</a></li>			
...
<?php
	...
	if ($even->getValue('idLieu') != 0)
	{
		...
		$lieu = "<a href=\"/pages//pages/lieu.php?idLieu=".$even->getValue('idLieu')."\" title=\"Voir la fiche du lieu : ".sanitizeForHtml($listeLieu['nom'])."\">".sanitizeForHtml($listeLieu['nom'])."</a>";
                
		$nom_lieu = '<a href="/pages/lieu.php?idL='.$even->getValue('idLieu').'" title="Voir la fiche du lieu : " >
...
<!-- <a href='#' onclick='window.open("http://www.ladecadanse.ch", "", "width=800px,height=300px");return false'>OPEN</A>-->
				...
				WHERE evenement_organisateur.idEvenement=".$get['idE']." AND
				 ...
				 $req = $connector->query($sql);
					while ($tab = $connector->fetchArray($req))
					{
						...
						echo '<li><strong><a href="/pages/organisateur.php?idO='.$tab['idOrganisateur'].'" title="Voir la fiche de l\'organisateur">'.$tab['nom'].'</strong></a>';
						if ( $tab['URL'] != '')
						{
		...
		<a class="signaler" href="/pages/evenement-report.php?idE=<?php echo $get['idE'] ?>" ><i class="fa fa-flag-o fa-lg"></i>
Signaler une erreur
		</a>
	 ...
	 idPersonne=".$even->getValue('idPersonne')." AND genre='lieu'");
			...
			{
			?>
				<a href="/pages/user.php?idP=<?php echo $even->getValue('idPersonne')?>"><?php echo $signature_auteur; ?></a>
			<?php
			}
			else
			{
		...
		if ((isset($_SESSION['Sgroupe']) && $_SESSION['Sgroupe'] <= 12))
		{
			...
			if (time() < datetime_iso2time(date_lendemain($detailsEven['dateEvenement'])." 06:00:01"))
			{
				if ($nb == 0)
				{
					echo '<a href="/pages/action_participation.php?action=ajouter&amp;idE='.$get['idE'].'"
				title="Ajouter">J\'y vais'.$icone['ajouter_date'].'</a>';
				}
				else
				{
					echo "<p>Vous avez prévu d'aller à cette événement";
					echo '<a href="/pages/action_participation.php?action=supprimer&amp;idE='.$get['idE'].'"
...
<form method="post" id="ajouter_editer"  class="submit-freeze-wait" action="/pages/multi-comment.php?action=insert&amp;&amp;element=evenement&amp;id=<?php echo $get['idE'] ?>">
...
<form action="/pages/user-login.php" method="post" style="width:auto;border-radius:3px">
    ...
    <a href="/pages/user-register.php" style='margin-left: 5px;'>Pas de compte ?</a>
</form>
</div>
<?php
}
...
echo "<h4><a href=\"/pages/lieu.php?idL=".$details_even['idLieu']."\">";

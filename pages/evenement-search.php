					...
					while ($tab_even = $connector->fetchArray($req_even))
					{
						...
						if ($tab_even['idEvenement'] == $no)
						{
								...
								if ($tab_even['idLieu'] != 0)
								{
									...
									$infosLieu = $listeLieu['determinant']." <a href=\"/pages/lieu.php?idL=".$tab_even['idLieu']."\" title=\"Voir la fiche du lieu : ".htmlspecialchars($listeLieu['nom'])."\" >".htmlspecialchars($listeLieu['nom'])."</a>".$salle;
							...
							?>
								...
								?>
								<h3><a href="<?php echo $url_site ?>/pages/evenement.php?idE=<?php echo $tab_even['idEvenement'] ?>" title="Voir la fiche de l'événement"><?php echo $titre ?></a></h3>
							<?php
...
/* 									foreach($tab_mots as $n => $mot)
							...
							<td><a href="/pages/agenda.php?courant=<?php echo $tab_even['dateEvenement']; ?>"><?php echo date_iso2app($tab_even['dateEvenement']) ?></a></td>
				...
				|| (isset($_SESSION['Saffiliation_lieu']) && !empty($tab_even['idLieu']) && $tab_even['idLieu'] == $_SESSION['Saffiliation_lieu'])
							...
							<a href="/pages/evenement-edit.php?action=editer&amp;idE=<?php echo $tab_even['idEvenement'] ?>" title="Éditer cet événement"><?php echo $iconeEditer; ?></a>
		...
		else
		{
			while($tab_even = $connector->fetchArray($req_even))
			{
					...
					if (!empty($tab_even['idLieu']))
					{
						...
						$infosLieu = $listeLieu['determinant']." <a href=\"".$url_site."/pages/lieu.php?idL=".$tab_even['idLieu']."\" title=\"Voir la fiche du lieu : ".htmlspecialchars($listeLieu['nom'])."\" >".htmlspecialchars($listeLieu['nom'])."</a>".$salle;
					...
					<td class="desc_even">
					<h3><a href="<?php echo $url_site ?>/pages/evenement.php?idE=<?php echo $tab_even['idEvenement'] ?>" title="Voir la fiche de l'événement"><?php echo $tab_even['titre'] ?></a></h3>
							<?php
								$maxChar = Text::trouveMaxChar($tab_even['description'], 50, 4);
								if (mb_strlen($tab_even['description']) > $maxChar)
								{

...
include($rep_absolu."pages/_menuorganisateurs.inc.php");
...
{
	$action_ajouter = '<li class="action_ajouter"><a href="/pages/evenement-edit.php?idO='.$get['idO'].'" title="ajouter un événement à ce lieu">Ajouter un événement de cet organisateur</a></li>';
}
...
{
	$action_editer = '<li class="action_editer"><a href="/pages/organisateur-edit.php?action=editer&amp;idO='.$get['idO'].'" title="Éditer cet organisateur">Modifier cet organisateur</a></li>';
}
...
//		$adresse = $organisateur->getValue('adresse').' - '.$organisateur->getValue('quartier');
		...
		if ($connector->getNumRows($req) > 0)
		{
			...
			{
				$lieux .= '<li><a href="/pages/lieu.php?idL='.$tab['idLieu'].'">'.$tab['nom']."</a></li>";
			}
			$lieux .= '</ul></li>';
		}
...
<?php
	$lien_rss_evenements = '<a href="/pages/rss.php?type=organisateur_evenements&amp;id='.$get['idO'].'" title="Flux RSS des prochains événements"><i class="fa fa-rss fa-lg" style="color:#f5b045"></i></a>';
?>
...
if ($evenements->getNbElements() > 0)
{
	...
	foreach ($genres_even as $g)
	{
		...
		if ($g == $get['genre_even'])
		{
			$menu_genre .= " class=\"ici\"><a href=\"/pages/organisateur.php?idO=".$get['idO']."&amp;genre_even=".urlencode($g)."#prochains_even\" title=\"".$g."\">";
			if ($g == "fête")
			{
				$g .= "s";
			}
		...
		{
			$menu_genre .= "><a href=\"/pages/organisateur.php?idO=".$get['idO']."&amp;genre_even=".$g."#prochains_even\" title=\"".$g."\">".$g;
			$menu_genre .= " (".$nb_even_genre.")";
			$menu_genre .= "</a>";
		}
	...
	foreach ($evenements->getElements() as $id => $even)
	{
		...
		if ($even->getValue('idLieu') != 0)
		{
			...
			$nom_lieu = "<a href=\"/pages/lieu.php?idL=".$even->getValue('idLieu')."\" title=\"Voir la fiche du lieu : ".htmlspecialchars($tab_lieu['nom'])."\" >".htmlspecialchars($tab_lieu['nom'])."</a>";
		}
		else
		{
	...
	<tr <?php if ($date_debut == $even->getValue('dateEvenement')) { echo "class=\"ici\""; } ?> class="evenement">
		...
		<td>
            <h3>
            <?php
            $titre_url = '<a href="/pages/evenement.php?idE='.$even->getValue('idEvenement').'" title="Voir la fiche de l\'événement">'.Evenement::titre_selon_statut(sanitizeForHtml($even->getValue('titre')), $even->getValue('statut')).'</a>';
            echo $titre_url; ?>
            </h3>

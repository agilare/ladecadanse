...
$eventCategories = ["fête", "cinéma", "théâtre", "expos", "divers"];
try
{
...
if ($get['entity'] == "event") {
	...
	$sql = "SELECT e.*, eloc.localite eLocalite, l.nom lNom, l.adresse lAdresse, l.quartier lQuartier, loc.localite lLocalite, l.URL lUrl
	 ...
	 WHERE dateEvenement = '" . $connector->sanitize($get['date']) . "' AND genre = '" . $connector->sanitize($get['category']) . "' AND e.statut NOT IN ('propose') AND e.region IN ('" . $connector->sanitize($get['region']) . "')"; // noctambus : don't covers 'rf', 'hs'

    if (!empty($get['endtime']))
        $sql .= "AND horaire_fin >= '" . $datetimeMinEnd->format('Y-m-d') . " " . $connector->sanitize($get['endtime']) . "' AND horaire_fin < '" . $datetimeMinEnd->format('Y-m-d') . " 06:00:01'"; // noctambus : covers from 01h to...
    ...
    $result = ['date' => $get['date'], 'events' => $events];
}

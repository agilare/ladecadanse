...
?>
 ...
 ?>
	...
	{
		echo '<li><a href="'.$url_site.'email_evenement.php?idE='.$get['idE'].'"
		title="Envoyer l\'événement par email">'.$icone['envoi_email'].'Envoyer à un ami</a></li>';
...
<!-- <a href='#' onclick='window.open("http://www.ladecadanse.ch", "", "width=800px,height=300px");return false'>OPEN</A>-->
		...
		<a class="signaler" href="<?php echo $url_site ?>signaler_erreur.php?idE=<?php echo $get['idE'] ?>" ><i class="fa fa-flag-o fa-lg"></i>
Signaler une erreur
		</a>
    ...
    width: 94%;">
		...
		if ((isset($_SESSION['Sgroupe']) && $_SESSION['Sgroupe'] <= 12))
		{
			...
			if (time() < datetime_iso2time(date_lendemain($detailsEven['dateEvenement'])." 06:00:01"))
			{
				if ($nb == 0)
				{
					echo '<a href="'.$url_site.'action_participation.php?action=ajouter&amp;idE='.$get['idE'].'"
				title="Ajouter">J\'y vais'.$icone['ajouter_date'].'</a>';
				}
				else
				{
					echo "<p>Vous avez prévu d'aller à cette événement";
					echo '<a href="'.$url_site.'action_participation.php?action=supprimer&amp;idE='.$get['idE'].'"
...
<h4><a href="<?php echo $url_site ?>evenement.php?idE=<?php echo $details_even['idEvenement'] ?>" title="Voir la fiche de l'événement">
<?php echo securise_string($details_even['titre'])."</a></h4>";

echo "<h4><a href=\"".$url_site."lieu.php?idL=".$details_even['idLieu']."\">";

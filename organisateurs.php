...
$page_titre = "Organisateurs d'événements culturels à Genève : associations, labels, collectifs";
...
foreach ($col->getElements() as $id => $fiche)
{
	...
	{
		$apercu = html_substr(textToHtml(htmlspecialchars($fiche->getValue('presentation'))), $maxChar, 2);
	}
	else
	{
		$apercu = textToHtml(htmlspecialchars($fiche->getValue('presentation')));
	}
	?>
